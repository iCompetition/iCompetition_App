...
def db_logScore(userName,eventNum,wkNum,pos,pnt,inc,lap,qual,diff,rights):
  db = _dbConnect("write",rights)
  cr = db.cursor()   
  cr.execute("insert into scoring values (" + eventNum + "," + wkNum + "," + userName + "," + pnt + "," + inc + "," + pos + ",0,'" + lap + "'," + qual + "," + diff + ");")
...
def db_pullEventFastLaps(eventNum,rights):
  db = _dbConnect("read",rights)
  cr = db.cursor()  
  cr.execute("select weekNum, min(laptime) from topLap where eventNum = " + str(eventNum) + " group by weekNum order by weekNum;")
  results = cr.fetchall()
  _dbClose(db,cr)
  return results  
